...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: ShowCaseWidget(builder: (context) => const MainScreen()),
    );
  }
}
...
class _MainScreenState extends State<MainScreen>
  ...
  // --- راهنمای اولیه (Coach marks) ---
  final GlobalKey _locationAccessKey = GlobalKey();
  final GlobalKey _firstDirectionsKey = GlobalKey();
  final GlobalKey _premiumButtonKey = GlobalKey();
  static const String _tutorialLocationKey = 'tutorial_location_shown';
  static const String _tutorialDirectionsKey = 'tutorial_directions_shown';
  static const String _tutorialPremiumKey = 'tutorial_premium_shown';
  bool _tutorialLocationShown = false;
  bool _tutorialDirectionsShown = false;
  bool _tutorialPremiumShown = false;
  ...
  Future<void> _initTimelineAndPurchase() async {
    ...
    // این متد هر بار (initState و هر pull-to-refresh) اجرا می‌شود؛ بدون این گارد
    // هر بار یک لیسنر جدید و هرگز حذف‌نشده به ValueNotifierهای سنگلتون اضافه می‌شد.
    if (!_purchaseListenersAttached) {
  ...
  void _checkAndLoadBannerAd() {
    if (PurchaseManager().isPremiumUser.value) {
      // اگر کاربر پرمیوم شده، بنر قبلی (در صورت وجود) را کاملاً آزاد کن
      _bannerAd?.dispose();
      _bannerAd = null;
    ...
    if (daysUsed < 3) return;

    // این متد هر بار (initState و هر pull-to-refresh) اجرا می‌شود؛
    // اگر بنر قبلی هنوز لود شده باشد، دوباره نساز (جلوگیری از لیک و ریکوئست اضافه)
  ...
  void _handleRestaurantScroll() {
    ...
    if (!_restaurantScrollController.position.hasContentDimensions) return;
    final nearBottom = _restaurantScrollController.position.maxScrollExtent -
            _restaurantScrollController.offset <
        50;
    if (nearBottom) _maybeShowPremiumTutorial();
  }
  ...
  Future<void> _loadFavoriteRestaurants() async {
    ...
    setState(() {
      ...
      _tutorialLocationShown = prefs.getBool(_tutorialLocationKey) ?? false;
      _tutorialDirectionsShown = prefs.getBool(_tutorialDirectionsKey) ?? false;
      _tutorialPremiumShown = prefs.getBool(_tutorialPremiumKey) ?? false;
    });
    _maybeShowLocationTutorial();
  }

  Future<void> _markTutorialShown(String prefsKey) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(prefsKey, true);
  }

  void _maybeShowLocationTutorial() {
    if (_tutorialLocationShown || currentUserPosition != null) return;
    _tutorialLocationShown = true;
    _markTutorialShown(_tutorialLocationKey);
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      // اول اپ رو یک ثانیه کامل روشن نشون بده، بعد هایلایت رو شروع کن
      await Future.delayed(const Duration(seconds: 1));
      if (!mounted) return;
      ShowCaseWidget.of(context).startShowCase([_locationAccessKey]);
    });
  }

  void _maybeShowDirectionsTutorial() {
    if (_tutorialDirectionsShown || currentUserPosition == null) return;
    _tutorialDirectionsShown = true;
    _markTutorialShown(_tutorialDirectionsKey);
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      await Future.delayed(const Duration(seconds: 1));
      if (!mounted) return;
      ShowCaseWidget.of(context).startShowCase([_firstDirectionsKey]);
    });
  }

  void _maybeShowPremiumTutorial() {
    if (_tutorialPremiumShown || PurchaseManager().isPremiumUser.value) return;
    _tutorialPremiumShown = true;
    _markTutorialShown(_tutorialPremiumKey);
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      await Future.delayed(const Duration(seconds: 1));
      if (!mounted) return;
      ShowCaseWidget.of(context).startShowCase([_premiumButtonKey]);
    });
  }
  ...
  Widget _buildRestaurantsList() {
    ...
    if (_selectedCity == kAroundMeCity) {
      filteredRestaurants = currentUserPosition == null
          ? []
          : _restaurantsAroundMe(allPossibleRestaurants, currentUserPosition!);
    } else {
   ...
   // --- ⭐️ جایگذاری کدهای جدید اینجاست ⭐️ ---
    ...
    fontSize: 16 * _fontScale,
          ...
          ...restaurantsToShow.asMap().entries.map((entry) {
            final r = entry.value;
            String detail = "";
            ...
            return _buildRestaurantItem(r, detail, isFirst: entry.key == 0);
          }),
  ...
  Widget _buildPaymentPrompt() {
    ...
    fontSize: 12 * _fontScale,
            ...
            Showcase(
              key: _premiumButtonKey,
              title: 'قدم ۳ از ۳',
              titleTextStyle: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15),
              description: 'در پایان، اینجا کلیک کنید تا به مدت ۶ ماه به تمام رستوران‌ها دسترسی کامل داشته باشید.',
              descTextStyle: const TextStyle(color: Colors.white, fontSize: 14),
              tooltipBackgroundColor: const Color(0xff004d99),
              overlayOpacity: 0.45,
              disposeOnTap: true,
              onTargetClick: isProcessing ? () {} : () => PurchaseManager().buyPremium(),
              child: ElevatedButton(
  ...
  Widget _buildRestaurantItem(Restaurant restaurant, String detail, {bool isFirst = false}) {
            ...
            SizedBox(
              ...
              child: Builder(
                builder: (context) {
                  ...
                  return Showcase(
                    key: _firstDirectionsKey,
                    title: 'قدم ۲ از ۳',
                    titleTextStyle: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15),
                    description: 'حالا اینجا کلیک کنید تا مسیر رسیدن به این رستوران روی نقشه باز شود.',
                    descTextStyle: const TextStyle(color: Colors.white, fontSize: 14),
                    tooltipBackgroundColor: const Color(0xff004d99),
                    overlayOpacity: 0.45,
                    disposeOnTap: true,
                    onTargetClick: () => _openDirections(restaurant),
                    child: button,
                  );
                },
              ),
  ...
  // شعاع جستجو را به تدریج افزایش می‌دهد تا حداقل ۳ رستوران پیدا شود
  // (مثلاً برای کاربری در کره جنوبی که رستوران نزدیکی ندارد)
  List<Restaurant> _restaurantsAroundMe(List<Restaurant> all, Position pos) {
    const List<double> radiusStepsKm = [
      50,
      100,
      250,
      500,
      ...
      5000,
      10000,
      20040, // تقریباً نیمی از محیط کره زمین: یعنی هر نقطه‌ای روی زمین
    ];

    List<Restaurant> withinRadius(double radiusKm) {
      final result = all.where((r) {
        return calculateDistance(pos.latitude, pos.longitude, r.lat, r.lon) <=
            radiusKm;
      }).toList();
      result.sort((a, b) {
        final distA = calculateDistance(pos.latitude, pos.longitude, a.lat, a.lon);
        final distB = calculateDistance(pos.latitude, pos.longitude, b.lat, b.lon);
        return distA.compareTo(distB);
      });
      return result;
    }

    for (final radius in radiusStepsKm) {
      final result = withinRadius(radius);
      if (result.length >= 3) return result;
    }
    // اگر کمتر از ۳ رستوران در کل دیتابیس وجود داشته باشد، همان‌ها نمایش داده می‌شوند
    return withinRadius(radiusStepsKm.last);
  }
  ...
  Future<void> _openAppleMaps({
    ...
    final String encodedDest = Uri.encodeComponent(destinationQuery);
    // Native Maps scheme opens Apple Maps on iPhone.
    final native = Uri.parse(
      'maps://?saddr=$origin&daddr=$encodedDest&dirflg=d',
    );
    // HTTPS always resolves to Apple Maps on iOS.
    final https = Uri.parse(
      'https://maps.apple.com/?saddr=$origin&daddr=$encodedDest&dirflg=d',
    );
  ...
  Future<void> _openGoogleMaps({
    ...
    final String encodedDest = Uri.encodeComponent(destinationQuery);
    final native = Uri.parse(
      'comgooglemaps://?saddr=$origin&daddr=$encodedDest&directionsmode=driving',
    );
  ...
  // مسیریابی بر اساس نام دقیق رستوران روی نقشه (name_map) به‌جای مختصات ذخیره‌شده،
  // چون مختصات هاردکدشده در فایل‌های دیتا گاهی نادقیق/اشتباه هستند.
  ...
  Future<void> _openDirections(Restaurant restaurant) async {
    ...
    void openMaps() {
      _openPlatformMaps(
        origin: origin,
        destinationQuery: restaurant.name_map,
      );
    }

    // اگر رستوران در حال حاضر بسته است، دیگه مسیریابی رو کامل قفل نکن؛
    // به کاربر بگو و بذار خودش تصمیم بگیره که مسیریابی رو ادامه بده یا نه.
    if (wh['valid'] == true && wh['closed'] == true) {
      final bool? proceed = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('این رستوران بسته است'),
          content: Text(
            'طبق ساعات کاری ثبت‌شده، این رستوران در حال حاضر بسته است. آیا با این حال مایلید مسیریابی انجام شود؟',
            style: TextStyle(fontSize: 12 * _fontScale),
          ),
          actions: [
            TextButton(
            ...
            ElevatedButton(
              ...
              child: const Text('بله، مسیریابی کن'),
            ),
          ],
        ),
      );
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 15),
            Showcase(
              key: _locationAccessKey,
              title: 'قدم ۱ از ۳',
              titleTextStyle: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15),
              description:
                  'اینجا کلیک کنید تا دسترسی به موقعیت مکانی فعال شود و جست‌وجوی رستوران‌های اطراف شما شروع شود.',
              descTextStyle: const TextStyle(color: Colors.white, fontSize: 14),
              tooltipBackgroundColor: const Color(0xff004d99),
              overlayOpacity: 0.45,
              // با یک تپ هم راهنما بسته بشه و هم عملِ واقعی دکمه اجرا بشه
              // (بدون این، اولین تپ فقط راهنما رو می‌بست و کاربر باید دوباره تپ می‌کرد)
              disposeOnTap: true,
              onTargetClick: () async {
                await _determinePosition();
                _maybeShowDirectionsTutorial();
              },
              child: ElevatedButton(
                onPressed: () async {
                  await _determinePosition(); // اجرای تابع قبلی خودتان
                  _maybeShowDirectionsTutorial();
                },
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xff004d99),
