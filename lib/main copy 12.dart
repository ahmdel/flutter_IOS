...
// --- متغیرهای مربوط به کشور و شهر ---
  ...
  final Map<String, List<String>> _countryCityMap = {
    ...
    "آلمان": ["برلین", "برمن", "دوسلدورف", "هامبورگ", "کلن", "مونیخ", "فرانکفورت", "بون", "شهرهای دیگر آلمان"], // "Hannover"
    ...
    "لهستان": ["ورشو"],
    "چک": ["پراگ"],
     //"آلمان": ["Berlin","Bremen", "Dusseldorf", "Hamburg", "Koln", "Munich", "Frankfurt", "Bonn", "DE Towns"],// "Hannover"
    // "فرانسه": ["Paris", "Strasbourg", "nice"],
    // "ایتالیا": ["Rome", "Milan"],
    // "اسپانیا": ["Barcelona", "Madrid","Malaga"],
    // "ترکیه": ["Istanbul", "Ankara"],
    // "کانادا": ["Vancouver", "Toronto"],
    // "آمریکا": ["New York", "Los Angeles"],
    // "استرالیا": ["Sydney", "Melbourne"],
    // "هلند": ["Amsterdam"],
    // "بلژیک": ["Brussels"],
    // "سوئیس": ["Zurich"],
    // "اتریش": ["Vienna"],
            ...
            child: Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  icon: const Icon(Icons.public, color: Color(0xff004d99), size: 20),
                  style: const TextStyle(color: Color(0xff004d99), fontWeight: FontWeight.bold, fontSize: 13, fontFamily: 'Tahoma'),
                  ...
                  items: _countryCityMap.keys.map<DropdownMenuItem<String>>((String value) {
            ...
            child: Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  isExpanded: true,
                  value: _selectedCity,
                  icon: const Icon(Icons.location_city, color: Colors.green, size: 20),
                  style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold, fontSize: 13, fontFamily: 'Tahoma'),
                  onChanged: (String? newValue) {
                    if (newValue != null) {
                      setState(() {
                  ...
                  items: _countryCityMap[_selectedCountry]!.map<DropdownMenuItem<String>>((String value) {
                    return DropdownMenuItem<String>(
                      value: value,
                      child: Text(value, overflow: TextOverflow.ellipsis),
                    );
...
Widget _buildRestaurantsList() {
    ...
    String internalCityName = _cityTranslationMap[_selectedCity] ?? _selectedCity;
      ...
      children: [
        ...
        if (filteredRestaurants.isEmpty)
          const Padding(
            ...
            child: Text("رستورانی برای این شهر یافت نشد.", style: TextStyle(fontWeight: FontWeight.bold)),
          )
        else
...
Widget _buildRestaurantItem(Restaurant restaurant, String detail) {
    ...
    bool isValid(String? value) {
      ...
      return cleanValue.isNotEmpty && cleanValue != 'n/a' && cleanValue != 'null' && cleanValue != '-';
    }
    ...
    Future<void> launchUrlish(String urlString, {bool isPhone = false}) async {
      ...
      try {
        if (await canLaunchUrl(url)) {
          await launchUrl(url, mode: isPhone ? LaunchMode.externalApplication : LaunchMode.platformDefault);
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
    ...
    Widget buildContactRow(IconData icon, String text, Color iconColor, VoidCallback? onTap) {
      return Padding(
              ...
              children: [
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 13, 
                      color: onTap != null ? Colors.blue.shade800 : Colors.grey.shade800,
    ...
    bool hasContactInfo = isValid(restaurant.address) || isValid(restaurant.phone) || isValid(restaurant.website);
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    restaurant.name,
                    textAlign: TextAlign.right,
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Color(0xff004d99)),
                  ),
                ),
                ...
                InkWell(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      color: _isFavorite(restaurant) ? Colors.red.shade50 : Colors.grey.shade200,
                      borderRadius: BorderRadius.circular(12),
                    ),
                    padding: const EdgeInsets.all(6),
                    child: Icon(
                      _isFavorite(restaurant) ? Icons.favorite : Icons.favorite_border,
                      color: _isFavorite(restaurant) ? Colors.red : Colors.grey.shade700,
                      size: 20,
                    ),
                  ),
                ),
            ...
            Wrap(
              ...
              children: [
                ...
                if (isValid(restaurant.price))
                  _buildInfoChip(Icons.euro_rounded, restaurant.price!, Colors.green.shade700, Colors.green.shade50),
                if (isValid(restaurant.parkingInfo))
                  _buildInfoChip(Icons.local_parking_rounded, restaurant.parkingInfo!, Colors.orange.shade700, Colors.orange.shade50),
              ],
            ),
                  ...
                  children: [
                    ...
                    if (isValid(restaurant.phone))
                      buildContactRow(
                        ...
                        () => launchUrlish("tel:${restaurant.phone!.replaceAll(' ', '')}", isPhone: true)
                      ),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text('مسیریابی روی نقشه گوگل', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 14)),
  ...
  Widget _buildInfoChip(IconData icon, String text, Color textColor, Color bgColor) {
  return Container(
      ...
      children: [
        ...
        Flexible(
          child: Text(
            ...
            overflow: TextOverflow.ellipsis, // ۳. سه نقطه گذاشتن در صورت طولانی شدن متن
            maxLines: 1,
          ),
        ),
  ...
  Map<String, dynamic> _evaluateWorkingHours(String? workingHours) {
    if (workingHours == null) return {'valid': false, 'closed': false, 'minutesUntilClose': null};
    final now = DateTime.now();

    // Pattern 1: "Closes 9:30 pm" (case-insensitive)
    final closesRegex = RegExp(r'Closes\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?', caseSensitive: false);
    ...
    if (m1 != null) {
      try {
        ...
        if (diff <= 0) return {'valid': true, 'closed': true, 'minutesUntilClose': null};
    ...
    final rangeRegex = RegExp(r'^(\d{1,2})(?::(\d{2}))?\s*-\s*(\d{1,2})(?::(\d{2}))?\s*\$');
