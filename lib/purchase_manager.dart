...
// ============================================================
// 🧪 فلگ تست Paywall — فقط برای توسعه/تست
// وقتی true باشه، بدون هیچ دست‌کاری‌ای در Firestore، اپ فوراً در حالت
// «کاربر رایگان با دوره‌ی رایگان تمام‌شده» قفل می‌شه (برای دیدن مستقیم
// صفحه‌ی Paywall/قفل کامل). بعد از اتمام تست، این مقدار رو حتماً به
// false برگردون تا رفتار عادی (واقعی، بر اساس Firestore) برگرده.
const bool kForceLockedPaywallForTesting = false;
// ============================================================
...
class PurchaseManager {
  ...
  final String premiumProductId = 'premiumunlock1';
  ...
  void _initPurchaseStream() {
    // جلوگیری از duplicate شدن subscription در صورت صدا زده شدن مجدد initialize()
    // (مثلاً هر بار pull-to-refresh)
    _purchaseSubscription?.cancel();
...
// ثبت وضعیت فعال‌سازی و شناسه دستگاه در فایربیس
  Future<void> _grantPremiumAccess(PurchaseDetails purchaseDetails) async {
    ...
    if (deviceId != null) {
      try {
        await FirebaseFirestore.instance.collection('premium_users').doc(deviceId).set({
          ...
          'expiryDate': expiryDate.toIso8601String(), // 👈 این خط باید اضافه شود
          'platform': Platform.isAndroid ? 'android' : 'ios',
        }, SetOptions(merge: true));

        // ۲. مصرف کردن (Consume) محصول در اندروید
    ...
    // هماهنگ کردن AppTimelineManager با خرید تازه، وگرنه چون premiumPurchaseDate
    // هیچ‌وقت ست نمی‌شه، calculateCurrentStatus() دوباره isPremiumUser رو false می‌کنه
  ...
  Future<void> checkPremiumStatus() async {
    if (kForceLockedPaywallForTesting) {
          ...
          if (data['is_premium'] == true) {
            ...
            if (data.containsKey('expiryDate')) {
              ...
              if (now.isBefore(expiryDate)) {
                 ...
                 // ۶ ماه تمام شده است
                 isPremiumUser.value = false;
                 // آپدیت دیتابیس که وضعیت کاربر به حالت عادی برگردد
                 await FirebaseFirestore.instance.collection('premium_users').doc(deviceId).update({
                   'is_premium': false
                 });
              }
            } else {
              // برای کاربرانی که قبلاً بدون محدودیت خریده‌اند و فیلد expiryDate ندارند
              isPremiumUser.value = true; 
            }
          }
        }
...
class AppTimelineManager {
  ...
  */
  ...
  // شناسه دستگاه یکپارچه برای اندروید و iOS (همان منطق initializeAndSync)
  Future<String> _getDeviceId() async {
  ...
  Future<void> savePremiumPurchaseToServer() async {
    ...
    // از set(merge:true) به‌جای update() استفاده می‌کنیم چون ممکن است
    // این داکیومنت هنوز در فایرستور وجود نداشته باشد (مثلاً اولین sync ناموفق بوده)
    // و update() روی داکیومنت ناموجود خطا می‌دهد.
  ...
  int get remainingFreeDays {
    if (kForceLockedPaywallForTesting) return 0;
  ...
  bool get isFreeTierExpired {
    if (kForceLockedPaywallForTesting) return true;
